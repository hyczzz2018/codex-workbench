...
class MockCoderAdapter(CoderAdapter):
    ...
    def handle_user_message(self, session_id: str, content: str) -> list[Event]:
        ...
        assistant_reply = (
            f"已收到你对 {session.current_stage} 阶段的补充。"
            "我会基于当前内容更新阶段产物。"
        )
    ...
    def confirm_stage(self, session_id: str, stage: StageType) -> list[Event]:
        ...
        events.append(
            self._make_stage_event(session_id, next_stage)
        )
        events.append(
            self._make_assistant_message(session_id, f"{stage} 已确认，正在生成 {next_stage} 阶段产物。")
        )
        events.append(self._make_artifact_event(session_id, next_stage))
        events.append(
            self._make_assistant_message(session_id, f"{next_stage} 已生成，请确认这一版是否继续。")
        )
    ...
    def _make_artifact_updated_event(self, session_id: str) -> Event:
        ...
        return Event(
            ...
            session_id=session_id,
            payload={"stage": str(artifact.stage), "content": artifact.content},
        )


mock_coder_adapter = MockCoderAdapter()
