...
function preferGatewayTranscriptMessage(current, next) {
  ...
  return nextText.length >= currentText.length ? next : current;
}
...
async function registerDevShelfGatewayResult() {
  ...
  try {
    const response = await fetch(`/api/dev-shelf/runs/${runId}/gateway/register-result`, {
      ...
      body: JSON.stringify({
        session_id: state.devShelf.gateway.sessionId,
        note: "用户在 Workbench 登记本轮实现结果。",
      }),
    });
    const result = await readJsonResponse(response);
    if (!response.ok) {
      throw new Error(result.detail || "登记实现结果失败");
    }
    state.devShelf.detail = result;
    control.lastResponse = null;
    control.message = "实现结果已登记，正在刷新下一步。";
    await refreshDevShelfSnapshot({ silent: false });
    ensureSelectedArtifact();
  } catch (error) {
    control.error = error.message;
...
function visibleDevShelfArtifacts() {
  ...
  const sortArtifacts = (artifacts) => [...artifacts].sort((left, right) => {
    const leftPriority = priority[left.artifact_id] ?? 20;
    const rightPriority = priority[right.artifact_id] ?? 20;
...
function buildWorkStatus(detail) {
  ...
  if (gateway.status?.status === "failed") {
    return {
      title: "pi-agent 执行失败",
      message: gateway.status.error || "请到页面底部的执行详情查看错误信息。",
    };
  }
  ...
  if (canRegisterImplementationResult(detail)) {
    return {
      title: "实现结果待登记",
      message: "pi-agent 已完成并写入项目文件；先登记实现结果，再让流程进入下一阶段。",
    };
  }
  if (previews.length) {
    return {
  ...
  if (gatewayCompletedForCurrentPacket(detail)) {
  ...
  if (primaryAction?.type === "gateway_start") {
    return {
      title: execution ? "可以开始代码实现" : "可以生成下一份中间产物",
      message: execution
...
function primaryActionForDetail(detail) {
  ...
  if (canStartGatewayForRun(detail)) {
    return { type: "gateway_start", label: isExecutionContext(detail) ? "开始代码实现" : "启动生成" };
  }
  if (canRegisterImplementationResult(detail)) {
    return { type: "register_result", label: "登记实现结果" };
  }
  return null;
}
...
function canRegisterImplementationResult(detail) {
  ...
  if (implementation && implementation.status && implementation.status !== "missing") {
    return false;
  }
...
function renderExecutionWorkbench(detail) {
  ...
  elements.devShelfExecutionRegisterButton.textContent = control.busy ? "登记中" : "登记实现结果";
  ...
  if (gatewayStatus === "completed") {
    if (implementation || producedCount > 0) {
      return {
        badge: "已登记",
        className: "ready",
        message: "pi-agent 已完成，且实现结果已经登记到流程产物。",
      };
    }
    return {
      badge: "待登记",
      className: "waiting",
      message: "pi-agent 已完成，但实现结果还没有登记为流程产物。",
    };
  }
  if (canStartGatewayForRun(detail)) {
    return {
...
function buildExecutionResultSummary(detail) {
  ...
  if (canRegisterImplementationResult(detail)) {
    lines.push(
      ...
      "## 当前阻塞",
      "",
      "- pi-agent 已结束，但实现结果未登记为 `implementation_result`。",
      "- 点击“登记实现结果”后，Workbench 会生成实现结果文档并刷新下一步。",
    );
...
function renderGatewayControls(detail) {
  ...
  elements.devShelfGatewayStartButton.textContent = control.busy ? "处理中" : execution ? "开始实现" : "启动";
  ...
  if (canRegister) {
    elements.devShelfGatewayControlStatus.textContent = "本轮实现已完成；先登记实现结果，不要重复启动同一轮 pi-agent。";
    elements.devShelfGatewayControlStatus.className = "gateway-control-status";
    return;
  }
  if (!canStartGateway) {
...
function renderCollabMessages(detail) {
  ...
  elements.devShelfCollabChat.innerHTML = "";
  if (!detail) {
    elements.devShelfCollabChat.innerHTML = '<p class="empty-state">请选择一个 run。</p>';
  ...
  if (!messages.length) {
    elements.devShelfCollabChat.innerHTML = '<p class="empty-state">还没有可展示的协作进展。</p>';
    state.devShelf.collabScroll.userDetached = false;
    return;
  }
  ...
  for (const message of messages) {
    ...
    const item = document.createElement("article");
    item.className = `collab-message collab-message-${message.role}${typing ? " is-typing" : ""}`;
    ...
    elements.devShelfCollabChat.appendChild(item);
  }
...
function buildCollabMessages(detail) {
  const messages = [{
    key: "system:next-action",
    role: "system",
    title: "Workbench",
    text: buildDevShelfNextAction(detail),
  }];
  ...
  appendCollabTranscriptMessages(messages, state.devShelf.gateway.transcript);
  if (!hasCollabAssistantTranscript(state.devShelf.gateway.transcript)) {
    messages.push(...buildCollabStreamMessages(state.devShelf.gateway.events || []));
  }

  if (isGatewayActivelyStreaming() && !hasLiveCollabProgress(messages)) {
    messages.push({
...
function shouldTypewriterCollabTranscriptMessage(message) {
  return Boolean(
    ...
    && (
      message.live_key
      || state.devShelf.collabTypewriter.entries[collabTranscriptMessageKey(message)]
    )
  );
}
...
function renderArtifactAction(artifact) {
  ...
  elements.devShelfArtifactActions.classList.toggle("hidden", !canConfirm && !artifactAction.error && !artifactAction.message);
  ...
  elements.devShelfArtifactActionStatus.textContent = canConfirm ? "确认后会刷新下一步。" : "";
  elements.devShelfArtifactActionStatus.className = "artifact-action-status";
}
...
function buildDevShelfNextAction(detail) {
  ...
  if (canRegisterImplementationResult(detail)) {
    return "下一步建议：登记实现结果，然后刷新进入后续阶段。";
  }
  const decision = packetDecision(detail);
